...
(defun define-relation (relation-name attribute-descriptor &rest keyword-list
  ...
  (block define-relation
	...
	(setf keyword-list (get-keyword-value-prereq '(dir dom def cardinality doc imp owner-id tup mod key sto
						       format) keyword-list)))
    ;;
    ;;  Determine if the implementation and storage structure specified for the current user relation are valid.
    ;;
	...
	(return-from define-relation nil))
    ;;
    ;;  Does a relation owned by this user already exist? If it does, a new one may not be defined until the current one is destroyed. In the
    ;; future, location transparency issues need to be introduced here.
    ;;
    (if (relation-exist-p relation-name)
	(return-from define-relation nil))
    ;;
    ;;  Define the tuples for the system-attribute relation. If an attribute descriptor is not specified, define a relation with one attribute.
    ;;
    ...
    (if (null
	 (do ((att-des attribute-descriptor (if (and (listp (cadr att-des))
	     ...
	     ((null att-des) t)
	   ;;
	   ;;  Determine if the attribute has already been defined for this relation
	   ;;
	   ...
	   ;;
	   ;;  If the second part of the attribute descriptor is provided, parse it and set the  appropriate values.
	   ;;
	...
	(return-from define-relation nil))
    ;;
    ;; Create the system-relation tuple which defines the new relation. This will temporarly be stored in a property list and will be inserted
    ;; later as described below. The manner of the implementation of the tuple depends on the method in which the system-relation relation is
    ;; implemented. The tuple created will be of the appropriate type. The attributes are the same regardless of implementation and can be
    ;; done for all types and then passed to the correct function.
    ;;
    ;;If any keywords and their values are provided (ex. in defrel call in RESTORE from QFASL) we should not use default. Specifically, we
    ;; should not set cardinality to 0.
    ;;
