...
(defun define-database (db-name &rest keyword-list
  ...
  "Define a new database:
   ...
   directory     - Name of the directory in which this database is to be
                   saved.
   environment   - Name of the environment to be associated with this
                   database.
   documentation - A string describing this database."
  (declare (ignore directory environment documentation))
  (let (dir-keyword-list keyword-values temp-validity-checking)
    (block define-database
      ;; If there is an activedb, determine if any relation has been
      ;; modified, if so do not do anything which would provoke those
      ...
      (cond ((and *parameter-checking* (active-database "defdb"))
	     (cond ((car (funcall
			   ...
			   *pkg-string*)
			  'system-relation *system-relation-attributes*
			  '("RELATION-NAME") *system-relation-key*
			  `(string-equal modifiedp "T") nil 'system-relation))
		    (cond (*provide-error-messages*
			   (format *standard-output*
				   (str "~%ERROR - ~s is the current "
					"database and it has modified "
					"relations")
				   *active-db*)
			   (format *standard-output*
				   (str "~%        Please resolve this "
					"conflict by either saving or "
      ...
      (cond ((not *parameter-checking*)
	     (setf db-name (string-upcase db-name)
		   keyword-list (get-keyword-value-prereq
				 '(dir doc env)
				 (de-nest-keyword-list keyword-list))))
	    ((null (setf db-name (validate-sym db-name t)))
	     (return-from define-database nil)))
      ;; Get the keyword values. Note, do not change the order of the
      ;; get-keyword-value keyword list. The code in this function
      ;; depend on this positional relationship !!!
      (setf
       ;; *save-directory* nil  -- don't do this -- MRR 5.27.87
       *save-user-id* nil
       keyword-values (get-keyword-value '(dir doc env) keyword-list)
       *save-directory* (get-directory keyword-list))
      ;;  If an environment was specified, activate it. Otherwise set
      ;;  the value of all variables to the system default values.
      (cond ((equal *environment-name*
		    (car (get-keyword-value '(env) keyword-list)))
	     t)
	    ((third keyword-values)
	     (setf *environment-name* (third keyword-values))
	     (cond ((listp *environment-name*)
		    (setf dir-keyword-list
			  (append '(dir) (cdr *environment-name*)))
	    ...
	    (t
	     ;; This part of the code needs to be redone as the
	     ;; following variables are no longer part of the
	     ;; environment
      ...
      (cond ((string-equal user-id "")
		 ...
		 (format *standard-output* "~%ERROR - You must be logged in "
			 "before defining a database"))
	     (return-from define-database nil)))
      (if *provide-status-messages*
	  (format *standard-output* "~%Defining database ~s" db-name))
      ;; Define the system relations. The first four system relations
      ;; are needed to define all of the others, therefore their
      ;; structures are defined and the tuple for the system-relation
      ;; relation is created but not inserted until all four have been
      ;; defined. Then the tuples will be inserted and the other
      ;; system relations will be defined.
      (setf *active-db* db-name
	    *save-user-id* user-id
	    user-id *pkg-string*
	    *domain-list* '("ANYP" "ATOM" "LISTP" "NUMBERP" "STRINGP")
	    ;; We will be needing this because all the information is
	    ;; not provided in the system-domain relation while we are
	    ;; defining the database. We will reset this to nil so
	    ;; that the user defined relations will refer to the
      ...
      (cond ((null (define-system-relation))
	     (if *provide-error-messages*
		 (format *standard-output*
			 "~%error - system-relation could not be defined"))
	     (setf *active-db* nil)
	     (return-from define-database nil)))
      ...
      (cond ((null (define-system-attribute))
	     (if *provide-error-messages*
		 (format *standard-output*
			 "~%error - system-attribute could not be defined"))
	     (setf *active-db* nil)
	     (return-from define-database nil)))
...
(defun define-system-attribute ()
  (setf *system-attribute-key* '("RELATION-NAME" "ATTRIBUTE-NAME")
	...
	*system-attribute-attributes* '("RELATION-NAME" "ATTRIBUTE-NAME"
					"DOMAIN-FUNCTION" "DEFAULT-VALUE"
					"DOC" "OWNER-ID")
