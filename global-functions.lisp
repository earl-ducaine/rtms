...
(t
 (setf reader-package-name *pkg-string*)))
  ...
  ;;
  ;;  Determine the attributes which have global variables defined for them. These will have to be take into
  ;; account in the where clause since it comes  from the reader.
  ;;
 (setf temp-attribute-list
       (mapcar #'(lambda (attr &aux global-attr)
   ;; This currently has a problem in that if a relaiton name is contained in the global package this will cause a
   ;; problem. The only solution seems to rewrite eval-where to parse the where-clause and locate the attributes
